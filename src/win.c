...
#pragma warning(disable: 4293)
    ...
    const uint64_t off64 = (uint64_t)off;
    const uint32_t dwFileOffsetLow = (uint32_t)(off64 & 0xFFFFFFFFULL);
    const uint32_t dwFileOffsetHigh = (uint32_t)((off64 >> 32) & 0xFFFFFFFFULL);
    const uint32_t protect = __map_mmap_prot_page(prot);
    const uint32_t desiredAccess = __map_mmap_prot_file(prot);

    const uint64_t maxSize = off64 + (uint64_t)len;

    const uint32_t dwMaxSizeLow = (uint32_t)(maxSize & 0xFFFFFFFFULL);
    const uint32_t dwMaxSizeHigh = (uint32_t)((maxSize >> 32) & 0xFFFFFFFFULL);
...
{
    DWORD newProtect = (DWORD)__map_mmap_prot_page(prot);
    DWORD oldProtect = 0;
